...
from sensor.constant.training_pipeline import SAVED_MODEL_DIR,SAVED_PREPROCESSOR_DIR
...
class TrainPipeline:
    ...
    def sync_artifact_dir_to_s3(self):
        try:
            ...
            logging.info("Syncing Artifacts Folder to S3 Bucket")
    ...
    def sync_saved_model_dir_to_s3(self):
        try:
            ...
            logging.info("Syncing Saved Models Folder to S3 Bucket")
            self.s3_sync.sync_folder_to_s3(folder = SAVED_MODEL_DIR,aws_buket_url=aws_buket_url)
        except Exception as e:
            raise SensorException(e,sys)
        
    def sync_saved_transformer_dir_to_s3(self):
        try:
            aws_buket_url = f"s3://{TRAINING_BUCKET_NAME}/{SAVED_MODEL_DIR}"
            logging.info("Syncing Saved Models Folder to S3 Bucket")
            self.s3_sync.sync_folder_to_s3(folder = SAVED_PREPROCESSOR_DIR,aws_buket_url=aws_buket_url)
        except Exception as e:
            raise SensorException(e,sys)
